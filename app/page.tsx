...
function SecretCard({ brand, eyebrow, title, email, password, meta }: CardProps) {
  ...
  return <article className={`service-card ${brand}`}>
    ...
    <div className="credentials">
      ...
      <div className="field"><div><span>Mot de passe</span><strong>&nbsp;</strong></div><button disabled aria-label={`Aucun mot de passe enregistré pour ${title}`}>&nbsp;</button></div>
    </div>
    <button className="copy-password" disabled>&nbsp;</button>
  </article>;
}
...
export default function Home() {
  return <main>
    ...
    <section className="cards" aria-label="Identifiants partagés">
      <SecretCard brand="mail" eyebrow="Compte principal" title="Notre adresse commune" email="abodisneyplus715@gmail.com" password="" meta="Le point d’entrée pour tous nos abonnements"/>
      <SecretCard brand="disney" eyebrow="Streaming" title="Disney+" email="abodisneyplus715@gmail.com" password="" meta="Formule Premium · Prochaine échéance le 1er septembre"/>
      <SecretCard brand="netflix" eyebrow="Streaming" title="Netflix" email="abodisneyplus715@gmail.com" password="" meta="Formule Premium · Prochaine échéance le 1er septembre"/>
    </section>
    <aside className="safety"><span aria-hidden="true">⌁</span><div><strong>Petit rappel entre amis</strong><p>Aucun mot de passe n’est enregistré sur cette page. Gardez-les dans un coffre partagé sécurisé.</p></div></aside>
